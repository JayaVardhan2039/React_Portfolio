...
/* =====================================================================
   Direct mirror of src/theme-light/hooks/useLightThemeEffects.js —
   see that file's header comment for the full mapping from the
   original vanilla-JS/iframe patterns to this React-owned-container
   version. The only real differences here are cosmetic/theme-specific:
     - the glow-cursor parallax moves 3 named orbs (#orb1/2/3) instead
       of looping over generic .hero-doodle elements
     - the background canvas draws soft circular particles instead of
       rotated confetti rectangles
     - the dial's active/inactive colors use this theme's white/dim
       palette instead of the light theme's coral/ink
   ===================================================================== */
export function useDarkThemeEffects(rootRef, { site, onHeroReady, onSectionChange }) {
  useEffect(() => {
    ...
    // stops the *second* mount from starting a duplicate init, and the
    // cleanup below is intentionally a no-op in normal use so it can
    // never cancel the first mount's pending GSAP setup (this component
    // is meant to live for the app's whole lifetime, so there's nothing
    // that actually needs tearing down in between).
    ...
    return () => {
      // Intentionally a no-op in normal use — see note above.
    };
...
function initAll(root, gsap, ScrollTrigger, { site, onHeroReady, onSectionChange }, cleanupFns) {
  ...
  function renderDial() {
    ...
    allItems.forEach((item, i) => {
      ...
      item.style.color = isActive ? '#ffffff' : `rgba(255,255,255,${(0.35 - t * 0.25).toFixed(2)})`;
  ...
  qa('.project').forEach((proj, i) => {
    ...
    gsap.fromTo(visual,
      ...
      {
        scale: 1, opacity: 0.9, rotate: i % 2 === 0 ? -8 : 8, duration: 1.3, ease: 'power3.out',
  ...
  const COLORS = ['#3df0ff', '#a368ff', '#ff4fd8', '#c9ff5c'];
  ...
  function drawBG() {
    ...
    particles.forEach((p) => {
      ...
      bgCtx.fillStyle = p.c; bgCtx.globalAlpha = p.a; bgCtx.fill();
    });
    bgCtx.globalAlpha = 1;
  ...
  function onWindowMouseMove(e) {
    ...
    if (orb1) gsap.to(orb1, { x: ox * 0.6, y: oy * 0.6, duration: 1.2, ease: 'power2.out' });
    if (orb2) gsap.to(orb2, { x: -ox * 0.8, y: -oy * 0.8, duration: 1.2, ease: 'power2.out' });
    if (orb3) gsap.to(orb3, { x: ox * 1.1, y: -oy * 0.5, duration: 1.2, ease: 'power2.out' });
  }
